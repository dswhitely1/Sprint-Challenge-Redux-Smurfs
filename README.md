...
- [x] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
        The Store is the source of all truth in an application.  With a global store, we do not need to manage data at the component level.  Instead we create action creators in the actions file that will dispatch our actions to reducers and based on the action type, we will then update our state in the redux store.
- [x] What is the difference between Application state and Component state? When would be a good time to use one over the other?
        Application state is the source of all data in the application.  Component state is UI State and usually controls visuals in that compoent or forms for controlled input.  Forms and a Modal would be very good examples of using UI State.
- [x] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?  Redux-Thunk is a middleware that allows us to perform async actions on our action creaters by returning a function instead of an object.  It's very good for handling calls out to an API.
